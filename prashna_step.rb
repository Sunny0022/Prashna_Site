...
And(/^Question is published successfully$/) do
  expect(page).to have_content('Question was created successfully.')
  click_link 'Logout'
end


And(/^I fill with title, contents and topics$/) do
  fill_in 'Title', :with => 'Who developed ruby language?'
  fill_in 'Content', :with => 'Yukihiro “Matz” Matsumoto, the creator'
